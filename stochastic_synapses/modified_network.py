...
class stochastic_model(nn.Module):
	...
	def forward(self, x):
		...
		x = F.relu(x.mm(torch.div(torch.mul(self.m1, torch.bernoulli(self.p1)), (self.p1))))
		x = F.relu(x.mm(torch.div(torch.mul(self.m2, torch.bernoulli(self.p2)), (self.p2))))
		x = F.softmax(x.mm(torch.div(torch.mul(self.m3, torch.bernoulli(self.p3)), (self.p3))))
		return x
